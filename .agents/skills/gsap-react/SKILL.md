...
function ControlledAnimation({ progress }) {
    ...
    useGSAP(
        () => {
            tl.current = gsap.timeline({ paused: true }).to('.element', { x: 500 }).to('.element', { y: 200 })
        },
        { scope: container }
    )
...
// Usage
function Component() {
    const boxRef = useAnimation((el) => gsap.from(el, { opacity: 0, y: 50, duration: 0.5 }))
...
function CountdownDigit({ value, label }) {
    ...
    return (
        <div className='digit-container'>
            <span ref={digitRef} className='digit'>
                {value}
            </span>
