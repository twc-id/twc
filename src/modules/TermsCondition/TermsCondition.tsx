...
const TermsCondition: React.FC<TermsConditionProps> = ({ locale }) => {
    const { t } = useTranslation('privacy-terms')
    const { data, isLoading } = usePages(locale === 'id' ? 'ketentuan-penggunaan' : 'terms-of-use', { _embed: true })
            ...
            <Container className='flex flex-col gap-20 py-[200px] xl:py-40'>
                <div className='flex flex-col gap-4'>
                    ...
                    <span className='xl:text-paragraph-7-desktop text-paragraph-7-mobile'>
                        <Trans i18nKey='terms.updated' components={{ strong: <strong /> }}>
                            {t('terms.updated', { time: formatDate(pages?.modified_gmt, 'D MMMM YYYY') })}
                        </Trans>
                    </span>
                </div>
                <div
